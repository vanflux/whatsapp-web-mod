...
export class ThemerMod {
  ...
  public static apply() {
    ...
    HTMLDivElement.prototype.setAttribute = function (...args: any) {
      if (args[0] === 'data-testid' && args[1] === 'conversation-panel-wrapper') {
    ...
    this.timers.push(setInterval(() => {
      ...
      if (appWrapperWeb) {
        const appWrapperWebSpanDivs = [...appWrapperWeb.querySelectorAll<HTMLDivElement>(':scope > span > div')];
        for (const appWrapperWebSpanDiv of appWrapperWebSpanDivs) {
          const statusPanel = appWrapperWebSpanDiv.querySelector<HTMLDivElement>('[data-testid^="status-"][data-testid$="-main-panel"]');
          if (statusPanel) {
            statusPanel.style.backdropFilter = 'blur(4px)';
            appWrapperWebSpanDiv.style.background = this.transparentRGBA(0.47);
          }
          const contactMenuDropdown = appWrapperWebSpanDiv.querySelector<HTMLDivElement>('[data-testid="contact-menu-dropdown"]');
          if (contactMenuDropdown) {
            appWrapperWebSpanDiv.style.backdropFilter = 'blur(4px)';
            appWrapperWebSpanDiv.style.background = this.transparentRGBA(0.47);
          }
          const mediaViewerModal = appWrapperWebSpanDiv.querySelector<HTMLDivElement>('[data-testid="media-viewer-modal"]');
          if (mediaViewerModal) {
            mediaViewerModal.style.backdropFilter = 'blur(4px)';
            appWrapperWebSpanDiv.style.background = this.transparentRGBA(0.47);
          }
        }
        const settingsDrawer = document.querySelector<HTMLDivElement>('[data-testid="settings-drawer"]');
        if (settingsDrawer) {
